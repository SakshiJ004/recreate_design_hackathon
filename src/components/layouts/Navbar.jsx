...
const Navbar = ({ visible = true }) => {
    return (
        <nav className={`hidden xl:block absolute left-1/2 -translate-x-[37%] bottom-[-38px] w-[80%] z-30 transition-all duration-500 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4 pointer-events-none'}`}>

            <div className="bg-gradient-to-r from-[#1F2F5E] to-[#5973b1] text-white px-10 py-2 shadow-lg nav-diagonal flex items-center justify-between">

                <ul className="flex items-center gap-10 text-[14px] font-medium tracking-wider cursor-pointer">
